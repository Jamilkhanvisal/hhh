...
db   db        db   dD d888888b d8b   db  d888b  
88   88        88 ,8P'   `88'   888o  88 88' Y8b 
88ooo88        88,8P      88    88V8o 88 88      
88~~~88 C8888D 88`8b      88    88 V8o88 88  ooo 
88   88        88 `88.   .88.   88  V888 88. ~8~ 
YP   YP        YP   YD Y888888P VP   V8P  Y888P  

\33[1;37m----------------------------------------------
â†’   OWNER     :  HAJI HAZRAT
â†’   FRIENDS   :  STANBOL RAYES
â†’   STATS     :  1.7
â†’   MACKED    :  BY RAYEES HAZRAT
\33[1;37m----------------------------------------------""")

boy = ['Ali khan', 'Haji Khan', 'Faisal Khan', 'Afzal Khan', 'Haider Khan', 'Suleman Khan', 'Lopar Halak', 'Wali ullah', 'Nadeem Khan', 'Nazeer Malik', 'Nazeer Jutt', 'Nazeer Rehmani', 'Safdar Malik', 'Intzar Khan', 'Saleem Malik', 'Abdullah Malik', 'Naseer Jutt', 'Muzammil Malik', 'Fiaz Ahmad', 'Asghar Ali', 'Shabeer Ahmad', 'Irfan Ali', 'Ahmad Gujjar']
...
class create:
    ...
    def start(self):
        os.system('clear')
        print (logo4)
        print ('[1] Boys name ids  hazrat map')
    ...
    # 'cookie': 'datr=_ou-ZCOsuxdq2Lhpdtm-pP9_; sb=_ou-ZD6wfuCT_7YUx7mgxZ_t; locale=en_GB; dpr=4; m_pixel_ratio=4; wd=980x1671; fr=08CvDxpmmx2V1H66W.AWXNFo2VN6--f9_7FgK22xRPFhU.Bkv_hs._c.AAA.0.0.BkxPNy.AWXWrIrqRv4',
             'sec-ch-prefers-color-scheme': 'light',
             'sec-ch-ua': '"(Not(A:Brand";v="99", "Chromium";v="116", "Opera";v="100"',
             'sec-ch-ua-full-version-list': '"(Not(A:Brand";v="99.0.0.0", "Chromium";v="116.0.5799.212", "Opera";v="100.0.3418.100"',
             'sec-ch-ua-mobile': '?0',
                        ...
                        for x in BeautifulSoup(po.text, 'html.parser').find_all('form', {'method': 'post'}):
                            if 'confirmation_event_location=cliff' in str(x):
                                ...
                                if 'checkpoint' in rex.url:
                                    cok = self.ses.cookies.get_dict()
                                    cp.append(email+passw)
                                    print ('\r\033[1;33m[H-CP] '+cok['c_user']+' | '+passw+'\033[0;97m     ')
                                else:
                                    coki = (";").join([ "%s=%s" % (key, value) for key, value in self.ses.cookies.get_dict().items() ])
                                    cok = self.ses.cookies.get_dict()
                                    print ('\r\033[1;32m[H-OK] '+cok['c_user']+' | '+passw+' | '+coki+'\033[0;97m     ')
